...
MESSAGE="login=Auctioneer&timing=undef1262813443.789997&password=testing&action=chat&otherb=none&target=Addon Update: $2&other=0&"
...
if [[ "$OS" != "Windows_NT" ]]; then
  ...
  aws s3 cp chatmod.js s3://rwk-assets/map_finder.js --cache-control "max-age=0" --acl public-read --profile personal
else
  ...
  aws s3 cp chatmod.js s3://rwk-assets/map_finder.js --cache-control "max-age=0" --acl public-read 
fi   
