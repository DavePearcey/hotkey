...
if [[ "$OS" != "Windows_NT" ]]; then
  ...
  aws s3 cp chatmod.js s3://rwk-assets/interface.js --cache-control "max-age=0" --acl public-read --profile personal
else
  ...
  aws s3 cp chatmod.js s3://rwk-assets/interface.js --cache-control "max-age=0" --acl public-read 
fi   
